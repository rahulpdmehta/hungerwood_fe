...
const OrderTracking = () => {
  ...
  // Sync UI step with order status (from polling or initial load).
  // MUST live above the conditional returns below so hook order stays stable.
  useEffect(() => {
    if (order?.status) {
  ...
  // Countdown timer (also kept above the early returns).
  useEffect(() => {
