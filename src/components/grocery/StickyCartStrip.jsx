...
export default function StickyCartStrip() {
  ...
  return (
    ...
    >
      <div className="flex -space-x-2.5 shrink-0">
        {thumbs.map((item, i) => (
          <div
            ...
            className="w-9 h-9 rounded-xl border-2 border-green-600 bg-white overflow-hidden flex items-center justify-center"
          >
            {item.image ? (
              <img
      ...
      <span className="text-xl font-bold leading-none rounded-full w-8 h-8 flex items-center justify-center bg-black/10 text-white" aria-hidden>›</span>
    </button>
  );
}
