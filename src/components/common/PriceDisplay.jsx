...
const PriceDisplay = ({
    ...
    return (
        <div className={`flex items-center gap-2 ${className}`}>
            {/* Discounted Price */}
            <span className={`text-[#7f4f13] ${sizeClasses[size]} font-bold`}>
                ₹{discountedPrice}
            </span>
            ...
            {hasDiscount && (
                <>
                    ...
                    {/* Discount Badge */}
                    <span className="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 text-2xs font-bold px-1.5 py-0.5 rounded">
                        {discount}% OFF
                    </span>
                </>
            )}
